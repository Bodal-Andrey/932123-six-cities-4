...
class ReviewsForm extends React.PureComponent {
  ...
  handleChange(evt) {
    ...
    switch (element.name) {
      case `rating`:
        this.setState({rating: element.value});
        break;
      case `review`:
    ...
    if (rating && (text.length >= 50 && text.length <= 300)) {
    ...
    return (
      <form onSubmit={this.handleSubmit} className="reviews__form form" action="#" method="post">
        ...
        <div className="reviews__button-wrapper">
          ...
          <button className="reviews__submit form__submit button" type="submit" disabled={submitState ? `` : `disabled`}>Submit</button>
        </div>
      </form>
    );
  }
